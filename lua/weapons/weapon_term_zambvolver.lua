...
if CLIENT then
    killicon.AddFont( "weapon_term_zambcopvolver", "HL2MPTypeDeath", ".", Color( 255, 80, 0 ) )
    language.Add( "weapon_term_zambcopvolver", SWEP.PrintName )
...
local function zambCopWhizPly( ply, start, endPos )
    local nextWhiz = ply.zambcop_NextBulletWhiz or 0
    if nextWhiz > CurTime() then return end

    local dist, nearestPoint = util.DistanceToLine( start, endPos, ply:GetShootPos() )
    if dist > 200 then return end
    ply.zambcop_NextBulletWhiz = CurTime() + 0.1
...
local function olReliableTrace( start, endPos )
    ...
    for _, ply in ipairs( player.GetAll() ) do
        if zambCopWhizPly( ply, start, endPos ) then return end
...
function SWEP:BulletCallback( iteration, attacker, bulletTrace, dmginfo, direction )
    ...
    zambcop_HandleDoor( self, bulletTrace )
...
function SWEP:PrimaryAttack()
    ...
    owner.zambcopBlockShooting = CurTime() + 1.5
    ...
    owner:FireBullets( {
        ...
        Callback = function( attacker, traceData, damageInfo )
            ...
            local oldCount = traceData.Entity.zambcopHitCounts or 0
            ...
            if hitSomething and owner.GetEnemy and IsValid( owner:GetEnemy() ) and hitEnt == owner:GetEnemy() then
                ...
                hitEnt.zambcopHitCounts = oldCount + 1
    ...
    if owner.zambGunSound then
        owner.zambGunSound:Stop()

    end

    owner.zambGunSound = CreateSound( owner, "Weapon_357.Single", filterAllPlayers )
    owner.zambGunSound:PlayEx( 1, 88 )

    timer.Simple( 0.05, function()
        if not IsValid( self ) then return end
        if not IsValid( owner ) then return end
        -- ECHO!
        local zambGunEcho = CreateSound( self, "Weapon_357.Single", filterAllPlayers )
        zambGunEcho:SetDSP( 22 )
        zambGunEcho:SetSoundLevel( 120 )
        zambGunEcho:PlayEx( 0.6, math.Rand( 55, 60 ) )
...
hook.Add( "terminator_blocktarget", "zambcop_ignoreunkillables", function( zambcop, target )
    local ignoreOverrides = zambcop.termIgnoreOverrides
...
if CLIENT then
    net.Receive( "weapon_term_zambcoprevolver.muzzleflash", function( len )
...
function SWEP:GetNPCBulletSpread( prof )
    local base = 0
    local owner = self:GetOwner()
    if IsValid( owner ) and owner.GetEnemy and owner.zambcopMaxUnequipRevolverDist and owner.DistToEnemy > ( owner.zambcopMaxUnequipRevolverDist * 1.5 ) then
        base = 0.3
