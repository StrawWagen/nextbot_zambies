...
function EFFECT:Init(data)
    ...
    if not IsValid(self.EntityToFollow) then
        self.EntityToFollow = nil
        self.EmitPos = data:GetOrigin()
    else
        self.Attachment = self.EntityToFollow:LookupAttachment("spine")
        if self.Attachment == 0 then
            self.Attachment = self.EntityToFollow:LookupAttachment("chest")
        end
        
        if self.Attachment and self.Attachment > 0 then
            self.EmitPos = self.EntityToFollow:GetAttachment(self.Attachment).Pos
        else
            self.Attachment = nil
            self.EmitPos = self.EntityToFollow:GetPos()
        end
    end
    ...
    local c1 = Color(255, 255, 255, 255)
    local c2 = Color(255, 255, 255, 255)
    ...
    self.Configs = {
        {
            ...
            EmitCount = 30,
            LifetimeMin = 2, LifetimeMax = 2.5,
            RadiusMin = 10, RadiusMax = 20,
            Color1 = c1, Color2 = c2,
            SpawnDistMax = 10,
            VelocityNoise = 15,
            Drag = 0.05,
            RadiusScaleStart = 1.0, RadiusScaleEnd = 0.5,
            AlphaFadeOut = 1.0,
            AnimRate = 1.0
        }
    }
...
function EFFECT:Think()
    ...
    if IsValid(self.EntityToFollow) then
        local currentPos
        if self.Attachment and self.Attachment > 0 then
            local attData = self.EntityToFollow:GetAttachment(self.Attachment)
            if attData then
                currentPos = attData.Pos + self.EntityToFollow:GetForward() * -10 + Vector(0, 0, 5)
            else
                currentPos = self.EntityToFollow:GetPos()
            end
        else
            currentPos = self.EntityToFollow:GetPos()
        end
